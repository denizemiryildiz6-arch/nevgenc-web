...
NevGenc.app = (()=>{
  ...
  function init(){window.addEventListener('hashchange',render);if(!location.hash)location.hash='#/anasayfa';bindSearch();document.getElementById('notifications-button').addEventListener('click',()=>{location.hash='#/anasayfa';toast('Duyurular ana sayfada güncel akış olarak gösteriliyor.')});render()}
