...
NevGenc.views = (()=>{
  ...
  async function profile(){
    ...
    if(!data) return `<section class="page"><div class="profile-gate card"><span class="icon-box">${icon.users}</span><span class="eyebrow">HESAP</span><h1 style="font-size:32px;margin-top:8px">Profil</h1><p>Takip edilen topluluklar ve duyuru tercihleri bu cihazda saklanıyor. Supabase Auth bağlandığında aynı bilgiler kullanıcı hesabıyla eşitlenir.</p><div class="profile-stats compact-stats"><div class="profile-stat"><strong>${followed.size}</strong><span>Takip</span></div><div class="profile-stat"><strong>${attending}</strong><span>Katılacağım</span></div></div><button class="button primary" data-action-message="Supabase Auth yapılandırıldığında giriş ekranı etkinleşecektir.">Giriş Yap</button></div></section>`;
    const p=data.profile||{};const name=p.full_name||data.user.email||'Kullanıcı';
    return `<section class="page"><div class="page-title"><div><span class="eyebrow">HESABIM</span><h1>Profil</h1></div></div><article class="profile-card card"><div class="profile-header"><div class="avatar">${e(name).charAt(0).toUpperCase()}</div><div><h2>${e(name)}</h2><p class="muted small">${e(p.department||data.user.email||'')}</p></div></div><div class="profile-stats"><div class="profile-stat"><strong>${Number(p.n_points||0)}</strong><span>N Puan</span></div><div class="profile-stat"><strong>${followed.size}</strong><span>Takip edilen</span></div><div class="profile-stat"><strong>${attending}</strong><span>Katılacağım</span></div><div class="profile-stat"><strong>${Object.keys(responses).length}</strong><span>Duyuru tercihi</span></div></div></article></section>`;
  }
