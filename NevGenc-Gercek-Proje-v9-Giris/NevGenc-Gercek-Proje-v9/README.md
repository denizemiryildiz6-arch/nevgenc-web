...
Bu sürümde kullanıcı ilk açılışta yalnızca **adını yazarak** giriş yapar; parola istenmez. Girilen ad ile takip ve duyuru tercihleri tarayıcının yerel depolamasında tutulur. Kurumsal kimlik doğrulama devreye alındığında aynı işlemler `community_follows` ve `announcement_responses` tabloları üzerinden doğrulanmış kullanıcı hesabına bağlanabilir.
...
Girilen ad `localStorage` üzerinde bu cihazda saklanır ve Profil ekranında gösterilir. Kullanıcı Profil sayfasından adını değiştirebilir veya çıkış yapabilir. Çıkış yapıldığında isim oturumu silinir ve giriş ekranı yeniden açılır.

Bu yöntem bir kimlik doğrulama sistemi değildir; ilk yayın ve kullanıcı deneyimi için geçici kişiselleştirme katmanıdır. Yetki gerektiren yönetim işlemleri veya hassas kullanıcı verileri bu isim oturumuna bağlanmamalıdır. İleride Supabase Auth veya kurumun tercih edeceği doğrulama sistemi eklendiğinde mevcut veri erişim katmanı korunarak güvenli hesaba geçilebilir.
